...
- [ ] Call `import_usd` (typed USD import, main-thread affinity — PIP-527):
  ```python
  import tempfile, os

  # Write a minimal USDA scene with a single cube
  usda_path = os.path.join(tempfile.gettempdir(), "smoke_cube.usda")
  ...
  body = json.dumps({
      ...
      "params": {"name": "import_usd",
                 "arguments": {"filepath": usda_path, "scale": 0.01}}
  }).encode()
  req = urllib.request.Request(
      "http://127.0.0.1:8765/mcp", data=body,
      headers={"Content-Type": "application/json",
  ...
  content = resp["result"]["content"]
  assert len(content) > 0
  result_text = json.loads(content[0]["text"])
  assert result_text.get("success") is True, f"import_usd failed: {result_text}"
  assert result_text.get("format") == "usd"
  assert result_text["context"]["imported_count"] >= 1
  assert "elapsed_ms" in result_text["context"]
  print("PASS: import_usd executed on main thread, imported USD assets")

  # Cleanup
  os.remove(usda_path)
  ```

- [ ] Verify imported USD object appears in Blender:
  ```python
  import bpy
  assert "SmokeUSDCube" in bpy.data.objects
  print("PASS: SmokeUSDCube imported from USD into Blender scene")
  ```
...
# 5. Main-thread import_usd (typed USD import — PIP-527)
import tempfile
usda_path = os.path.join(tempfile.gettempdir(), "smoke_cube.usda")
with open(usda_path, "w") as f:
    f.write("""#usda 1.0
...
check("import_usd (main-thread)", "THREAD_AFFINITY_UNAVAILABLE" not in json.dumps(result))
# Parse the tool result and verify typed output
tool_text = result.get("result", {}).get("content", [{}])[0].get("text", "{}")
tool_result = json.loads(tool_text)
check("import_usd format=usd", tool_result.get("format") == "usd")
check("import_usd imported_count>=1", tool_result.get("context", {}).get("imported_count", 0) >= 1)
check("import_usd has elapsed_ms", "elapsed_ms" in tool_result.get("context", {}))
os.remove(usda_path)
