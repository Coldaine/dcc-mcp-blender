...
## Broadside Skills (Project Broadside warship pipeline)

Broadside-specific skill package added by the Coldaine fork. `broadside-spatial-truth` is the functional Broadside skill. Further Broadside skills (component-modeling, ship-assembly, materials-and-texturing, articulation-and-rigging, visual-review, validation-and-publish) remain a design target only — see `docs/broadside/SELECTION.md` and `docs/broadside/skills.yaml`; they are not built in-tree. Until then, compose the bundled upstream skills above directly.

| Skill | Stage | Purpose | Default-load policy | Side-effect profile | Discovery terms |
|---|---|---|---|---|---|
| `broadside-spatial-truth` | validation, assembly, geometric-truth | World-space measurement and assertions: distance, dimensions, gap, overlap, alignment; assert contact, dimensions, containment, symmetry, proportion. | Load for warship assembly verification (turret-on-barbette, barrel parallelism/equal-length, director centering, containment, port/starboard mirror, armor gaps). | Read-only measurement and assertion; no scene mutation. | measure distance, measure dimensions, gap, overlap, alignment, assert contact, containment, symmetry, proportion, barbette seating, barrel parallelism, mirror match |
